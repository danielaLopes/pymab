...
class LinUCBPolicy(ContextualPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        validate_positive(alpha, name="alpha")
        validate_positive(l2, name="l2")
    ...
    def upper_confidence_bounds(self, context: FloatArray) -> FloatArray:
        ...
        for arm in range(self.n_arms):
            ...
            x = context[arm]
            solved_x = np.linalg.solve(self.a[arm], x)
            uncertainty = np.sqrt(max(float(x @ solved_x), 0.0))
    ...
    def recommend_action(self, *, context: FloatArray) -> int:
        ...
        means = np.array(
            [
                np.linalg.solve(self.a[arm], self.b[arm]) @ context[arm]
                for arm in range(self.n_arms)
            ],
            dtype=float,
        )
...
class LinearThompsonSamplingPolicy(ContextualPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        validate_positive(exploration_scale, name="exploration_scale")
        validate_positive(l2, name="l2")
    ...
    def select_action(self, *, context: FloatArray, rng: np.random.Generator) -> int:
        ...
        for arm in range(self.n_arms):
            eye = np.eye(self.n_features, dtype=float)
            a_inv = np.linalg.solve(self.a[arm], eye)
...
class LogisticContextualBanditPolicy(ContextualPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        validate_probability(epsilon, name="epsilon")
        validate_positive(learning_rate, name="learning_rate")
        validate_positive(l2, name="l2", allow_zero=True)
        self.epsilon = float(epsilon)
        self.learning_rate = float(learning_rate)
        self.l2 = float(l2)
        self.theta: FloatArray
    ...
    def recommend_action(self, *, context: FloatArray) -> int:
        return int(np.argmax(self.predicted_probabilities(context)))
