...
def test_bootstrap_band_validation_and_single_replicate() -> None:
    ...
    lower, upper = _bootstrap_band(values, n_resamples=5)
...
def test_bootstrap_band_chunking_preserves_results() -> None:
    ...
    unchunked = _bootstrap_band(
        values,
        config=BootstrapBandConfig(
            n_resamples=40,
            seed=3,
            max_output_elements=10_000,
        ),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"n_resamples": 100, "max_output_elements": 50},
    ],
)
def test_bootstrap_band_config_validation(kwargs) -> None:
    with pytest.raises((TypeError, ValueError)):
        BootstrapBandConfig(**kwargs)


def test_bootstrap_band_rejects_mixed_configuration() -> None:
    with pytest.raises(ValueError, match="cannot be combined"):
        _bootstrap_band(
            np.ones((2, 2, 1)),
            config=BootstrapBandConfig(n_resamples=2),
            seed=1,
        )
